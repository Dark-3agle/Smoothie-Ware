...
void Endstops::home_xy()
{
    if(axis_to_home[X_AXIS] && axis_to_home[Y_AXIS]) {
        float delta[3] {homing_axis[X_AXIS].max_travel, homing_axis[Y_AXIS].max_travel};
        // /Home XY first so as not to slow them down by homing Z at the same time
        float rahul[3]={0,0,5};// we added this s othat we can mve extruder up and to indivisual home .
        if(homing_axis[X_AXIS].home_direction||homing_axis[Y_AXIS].home_direction) rahul[Z_AXIS]=rahul[Z_AXIS];
        THEROBOT->delta_move(rahul, homing_axis[Z_AXIS].fast_rate, 3);
        ...
        float rahul[3] {0, 0,5}; // we added this so extruder moves up before x homes
        if(homing_axis[Z_AXIS].home_direction) rahul[Z_AXIS]= rahul[Z_AXIS];
        THEROBOT->delta_move(rahul, homing_axis[Z_AXIS].fast_rate, 3);
        // wait for Z
        THECONVEYOR->wait_for_idle();
		// now home X only
        float delta[3] {homing_axis[X_AXIS].max_travel, 0, 0};
        if(homing_axis[Z_AXIS].pin_info->triggered) rahul[Z_AXIS]= rahul[Z_AXIS];
        THEROBOT->delta_move(rahul, homing_axis[Z_AXIS].fast_rate, 3);
        if(homing_axis[X_AXIS].home_direction) delta[X_AXIS]= -delta[X_AXIS];
